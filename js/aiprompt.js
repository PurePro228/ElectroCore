...
(function (global) {
  ...
  function build() {
    ...
    /* -------------------------- процессор --------------------------- */
    rule('=');
    add(' 7. ТРИ ВЫЧИСЛИТЕЛЬНЫЕ МАШИНЫ');
    rule('=');
    add();
    add('В наборе их три, и путать их нельзя:');
    add();
    ...
    add();
    add('  board32  Плата EC-32. Тот же процессор EC-8, но на плате: рядом');
    add('           уже стоят память и расширитель портов, а наружу');
    add('           выведены шестнадцать линий P0…P15 двумя портами по');
    add('           восемь. Это наш аналог готового модуля вроде ESP32.');
    add('           Программа лежит в параметре "code".');
    add();
    add('Берите cpu8, когда хватает четырёх линий, board32 — когда линий');
    add('нужно много (клавиатура, индикатор без драйвера, десяток');
    add('светодиодов), и cpu_bus, когда человек просит показать настоящую');
    add('шину, внешнюю память или устройство компьютера.');
    add();
    add('Программа пишется обычным текстом. В JSON переводы строк');
    add('записывай как \\n.');
    add();
    add('Устройство всех трёх машин одно: восемь бит, регистры A и B,');
    add('флаги нуля (Z) и переноса (C). У EC-8 и платы память своя,');
    add('у EC-8B — внешняя.');
    add();
    rule('-');
    ...
    add('              NOT RET OUT IN DIR OUTB INB DIRB HLT');
    add();
    ...
    add();
    add('У платы board32 портов два, по восемь линий в каждом:');
    add('  P0…P7    первый порт  — DIR, OUT, IN');
    add('  P8…P15   второй порт  — DIRB, OUTB, INB');
    add('Работают они одинаково, просто команды разные. У cpu8 и cpu_bus');
    add('второго порта нет, и команды OUTB, INB, DIRB там бесполезны.');
    add();
    add('«метка+N» — это адрес на N байт дальше метки. Так правят операнд');
    add('у своей же команды и достают байт из таблицы по номеру:');
    add('  LD номер / ADDI таблица / ST взять+1 / взять: LD таблица');
    add('Первый байт команды — код, второй — операнд, поэтому и +1.');
    add();
    add('СИСТЕМА КОМАНД:');
    add();
    ...
    rule('=');
    add();
    add('У микроконтроллера EC-8 всего четыре линии порта. Когда выходов');
    add('не хватает, берут плату board32 с шестнадцатью линиями или те же');
    add('микросхемы, что и в жизни.');
    add();
    add('seg7 — ОДНА ЦИФРА');
    add();
    ...
    add();
    add('keypad16 — КЛАВИАТУРА 4×4');
    add();
    add('Шестнадцать кнопок и восемь выводов: C4, C3, C2, C1, R1, R2, R3,');
    add('R4 — это выводы 0…7 по порядку. Кнопка S<n> соединяет ряд');
    add('R((n-1)/4+1) со столбцом C((n-1) mod 4+1): S1 — R1 и C1,');
    add('S6 — R2 и C2, S16 — R4 и C4.');
    add();
    add('Опрос: ряды делают выходами, столбцы — входами. Подают единицу');
    add('на один ряд, читают столбцы; если какой-то столбец стал единицей,');
    add('нажата кнопка на их пересечении. Потом единицу переносят на');
    add('следующий ряд. Восемь линий платы board32 как раз для этого:');
    add('  board32 11…14 (P8…P11)  →  keypad16 4…7 (R1…R4)   ряды');
    add('  board32 15…18 (P12…P15) →  keypad16 3, 2, 1, 0 (C1…C4)  столбцы');
    add('Подтяжка столбцов не нужна: входы платы уже притянуты к нулю.');
    add('Готовый пример — схема «Клавиатура 4×4 и плата EC-32».');
    add();
    ...
    add('  [ ] линии второго порта (P8…P15) настроены командой DIRB,');
    add('      а не DIR — и наоборот');
